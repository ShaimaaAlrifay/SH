...
const GhostCursor = ({
  ...
  const fragmentShader = `
    ...
    /* lighter/darker variants of the SAME hue, not white/blue —
       keeps the rendered color true to the "color" prop */
    vec3 tint1(vec3 base){ return clamp(base * 1.35 + 0.05, 0.0, 1.0); }
    vec3 tint2(vec3 base){ return base * 0.55; }
    ...
    void main() {
      ...
      colorAcc = clamp(colorAcc, 0.0, alphaAcc);
      ...
      gl_FragColor = vec4(colorAcc * iOpacity * edgeMask, outAlpha);
    }
  `;
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({
      ...
      // the scene's own fragment shader already outputs premultiplied
      // color (rgb is pre-multiplied by alpha in blob()); matching that
      // here avoids needing a separate unpremultiply pass, which would
      // blow up to white wherever bloom bleeds color into near-zero-alpha
      // pixels (rgb / tiny-alpha explodes, then gets clamped to white).
    ...
    // bloomRadius is clamped: UnrealBloomPass blends its mip chain using
    // radius as a 0..1 lerp factor internally, so values outside that
    // range (e.g. > 1) push every mip toward maximum blur and dissolve
    // the trail into a single indistinct, oversaturated wash.
    const safeBloomRadius = Math.min(Math.max(bloomRadius, 0), 1);
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), bloomStrength, safeBloomRadius, bloomThreshold);
